...
if(cluster_group_get_level(group)==0)
	return block_map_item_group_get_first_item((block_map_item_group_t*)group);
return ((block_map_parent_group_t*)group)->first;
}
...
{
if(cluster_group_get_level(group)==0)
	return block_map_item_group_get_last_item((block_map_item_group_t*)group);
return ((block_map_parent_group_t*)group)->last;
}
...
if(cluster_group_get_level(group)==0)
	{
	added=block_map_item_group_add_block(heap, (block_map_item_group_t*)group, info, passive);
	}
else
	{
	added=block_map_parent_group_add_block(heap, (block_map_parent_group_t*)group, info, again, passive);
	}
if(!passive)
...
if(cluster_group_get_level(group)==0)
	{
	got=block_map_item_group_get_block(heap, (block_map_item_group_t*)group, min_size, info, passive);
	}
else
	{
	got=block_map_parent_group_get_block(heap, (block_map_parent_group_t*)group, min_size, info, passive);
	}
if(!passive)
	cluster_group_set_locked((cluster_group_t*)group, false);
return got;
...
if(cluster_group_get_level(group)==0)
	{
	removed=block_map_item_group_remove_block(heap, (block_map_item_group_t*)group, info, passive);
	}
else
	{
	removed=block_map_parent_group_remove_block(heap, (block_map_parent_group_t*)group, info, passive);
	}
if(!passive)
	cluster_group_set_locked((cluster_group_t*)group, false);
return removed;
...
if(item->index)
	...
	offset_index_open(&index, item->offset);
	added=offset_index_add_offset(heap, &index, info->offset);
	item->offset=(size_t)index.root;
	}
else
	{
	offset_index_t index;
...
if(item->index)
	...
	offset_index_t index;
	offset_index_open(&index, item->offset);
	info->offset=offset_index_remove_offset_at(heap, &index, 0);
	item->offset=(size_t)index.root;
	if(!item->offset)
...
bool block_map_item_group_remove_block(heap_handle_t heap, block_map_item_group_t* group, heap_block_info_t const* info, bool passive)
{
assert(!passive);
bool exists=false;
uint16_t pos=block_map_item_group_get_item_pos(group, info->size, &exists);
...
if(item->index)
	...
	offset_index_t index;
	offset_index_open(&index, item->offset);
	offset_index_remove_offset(heap, &index, info->offset);
	item->offset=(size_t)index.root;
	if(!item->offset)
		block_map_item_group_remove_item_at(group, pos, passive);
	}
...
group->first=block_map_group_get_first_item(child);
group->last=block_map_group_get_last_item(child);
...
block_map_item_t* first_ptr=NULL;
block_map_item_t* last_ptr=NULL;
int16_t empty=0;
while(start<end)
	{
	uint16_t pos=start+(end-start)/2+empty;
	first_ptr=block_map_group_get_first_item(group->children[pos]);
	if(first_ptr==NULL)
		{
		if(empty<0)
			{
	...
	empty=0;
	if(first_ptr->size>size)
		{
		end=pos;
		continue;
		}
	last_ptr=block_map_group_get_last_item(group->children[pos]);
	if(last_ptr->size<size)
		{
		start=pos+1;
		continue;
		}
...
if(start>0)
	{
	first_ptr=block_map_group_get_first_item(group->children[start]);
	if(first_ptr==NULL||first_ptr->size>size)
		{
...
if(start+1<child_count)
	{
	last_ptr=block_map_group_get_last_item(group->children[start]);
	if(last_ptr==NULL||last_ptr->size<size)
		return 2;
	}
return 1;
}
...
bool block_map_parent_group_add_block(heap_handle_t heap, block_map_parent_group_t* group, heap_block_info_t const* info, bool again, bool passive)
{
assert(!passive);
...
bool block_map_parent_group_remove_block(heap_handle_t heap, block_map_parent_group_t* group, heap_block_info_t const* info, bool passive)
{
assert(!passive);
uint16_t pos=0;
...
for(uint16_t pos=0; pos<child_count; pos++)
	{
	group->first=block_map_group_get_first_item(group->children[pos]);
	if(group->first!=NULL)
		break;
	}
for(uint16_t pos=child_count; pos>0; pos--)
	{
	group->last=block_map_group_get_last_item(group->children[pos-1]);
	if(group->last!=NULL)
		break;
	}
}
...
if(!block_map_lift_root(heap, map))
	return false;
return block_map_group_add_block(heap, map->root, info, true);
}
